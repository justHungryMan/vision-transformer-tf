...
def _decode_and_random_crop(image_bytes, image_size, min_area=0.08):
...
def preprocess_for_train(
    image_bytes, use_bfloat16, image_size, img_augment, min_area=0.08
):
    ...
    """
    image = _decode_and_random_crop(image_bytes, image_size, min_area=min_area)
    image = _flip(image)
    ...
    if img_augment:
        tf.logging.info("Apply AutoAugment policy %s", img_augment.augmentation_name)
        image = img_augment.distort(image)
...
def preprocess_image(
    ...
    if is_training:
        return preprocess_for_train(
            image_bytes, use_bfloat16, image_size, img_augment, min_area=min_area
        )
    else:
...
def distort_crop(
    ...
    bad = _at_least_x_are_equal(original_shape, tf.shape(image), 3)
    image = tf.cond(bad, lambda: center_crop(image, size), lambda: image)
